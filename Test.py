...
optimizer = mo.ModelOptimizer(model, mo.HyperparameterGenerator('categorical_crossentropy'), train_x, train_y)
allModels = optimizer.Optimize()
[print (report.accuracy) for report in allModels]
bestmodel = allModels[0].model


# Predict results
pred = bestmodel.predict(test_x)                                                    # Use the model to predict the output of all the test samples (test samples has no "true" label so we don't know what they are)
